...
void ADC0_InitSWTriggerSeq3_Ch9(void){ 
  ...
  GPIO_PORTE_DIR_R &= ~0x01;      // 2) make PE4 input
  GPIO_PORTE_AFSEL_R |= 0x01;     // 3) enable alternate function on PE4
  GPIO_PORTE_DEN_R &= ~0x01;      // 4) disable digital I/O on PE4
  GPIO_PORTE_AMSEL_R |= 0x01;     // 5) enable analog functionality on PE4
...
//  SYSCTL_RCGC0_R |= 0x00010000; // 1) activate ADC0 (legacy code)
  ...
  GPIO_PORTE_DIR_R &= ~0x07;      // 3) make PE4 PE5 PE2 input
  GPIO_PORTE_AFSEL_R |= 0x07;     // 4) enable alternate function on PE4 PE5 PE2
  GPIO_PORTE_DEN_R &= ~0x07;      // 5) disable digital I/O on PE4 PE5 PE2
                                  // 5a) configure PE4 as ?? (skip this line because PCTL is for digital only)
  GPIO_PORTE_PCTL_R = GPIO_PORTE_PCTL_R&0xFFFFF000;
  GPIO_PORTE_AMSEL_R |= 0x07;     // 6) enable analog functionality on PE4 PE5 PE2
  ADC0_PC_R &= ~0xF;              // 8) clear max sample rate field
  ADC0_PC_R |= 0x1;               //    configure for 125K samples/sec
  ADC0_SSPRI_R = 0x3210;          // 9) Sequencer 3 is lowest priority
  ADC0_ACTSS_R &= ~0x0004;        // 10) disable sample sequencer 2
  ADC0_EMUX_R &= ~0x0F00;         // 11) seq2 is software trigger
  ADC0_SSMUX2_R = 0x0213;         // 12) set channels for SS2
  ADC0_SSCTL2_R = 0x0060;         // 13) no TS0 D0 IE0 END0 TS1 D1 <IE1,END1,TS2,D2> yes <IE2 END2>
...
// data[0] is ADC1 (PE2) 0 to 4095
// data[1] is ADC8 (PE5) 0 to 4095
// data[2] is ADC9 (PE4) 0 to 4095
void ADC_In189(uint32_t data[3]){ 
  ADC0_PSSI_R = 0x0004;            // 1) initiate SS2
  while((ADC0_RIS_R&0x04)==0){};   // 2) wait for conversion done
//  data[2] = ADC0_SSFIFO2_R&0xFFF;  // 3A) read first result		
  data[1] = ADC0_SSFIFO2_R&0xFFF;  // 3B) read second result
  data[0] = ADC0_SSFIFO2_R&0xFFF;  // 3C) read third result
